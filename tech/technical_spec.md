...
7. [指標計算](#7-指標計算)
   ...
   - [I6 語義熵時間序列](#i6-語義熵時間序列-semantic-entropy)
...
- **非補償性升級（Non-compensatory Escalation）**：Tripwire 一旦觸發，無論其他指標分數多低，群組都會排到最前面。單一嚴重訊號不能被其他良好指標「平均掉」。
- **分層執行（Tiered Execution）**：便宜的規則型指標（I1、I2、I4、I6）對所有群組持續計算；昂貴的 LLM 指標（I3）只在規則層已發出警示時才啟動。
- **業務時間感知**：時間類指標（I1、I2）僅計算業務時間內的分鐘數，避免週末或深夜空窗期誇大延遲數字。I3 與 I6 使用掛鐘時間（見各節說明）。
...
LINE 匯出 .txt
  ...
  [enrichment.py]      對話行為分類、情緒評分、升級標記
       │
       ▼
  [metrics.py]         計算 I1 / I2 / I4 / I5 / I6 / Tripwire / Composite
       │
  ...
  [dashboard.py]       排名 + 終端機輸出
       │
       ▼
  [report_writer.py]   PDF 報告產生（含 I6 sparkline 圖表）
```
...
| 類別 | 詞彙範例 |
|------|---------|
| 情緒狀態 | 失望、不滿、不愉快、不開心、生氣、不耐煩、崩潰、心寒 |
| 品質評價 | 不好、不合理、不專業、無法接受、離譜、差勁、不符合 |
| 信任承諾 | 難以相信、不信任、承諾跳票、一再、每次都、交代 |
| 進度延誤 | 嚴重落後、進度落後、一再延誤、遲遲、久等 |
| 服務態度 | 態度、沒有回應、都沒有人、沒人理 |
| 升級意圖 | 退費、退款、退訂、投訴、換人 |
| 其他 | 困擾、擔心、疑慮、拍謝、抱歉 |
...
所有 severity 值範圍為 `[0.0, 1.0]`，透過線性插值函數 `_age_to_severity()` 轉換：
...
**代表意義**：近 72 小時內，客戶訊息中負面情緒訊息的佔比。  
使用 72 小時視窗（而非 24 小時）以確保近期的危機事件不會因時間略過就消失。

**計算流程**：

1. 篩選 `role == "customer"` 且 `timestamp >= now - 72h` 且 `not is_sticker` 的訊息
2. 統計其中 `sentiment < -0.1` 的訊息數量
3. `i4_neg_ratio = 負面訊息數 / 客戶訊息總數`
4. Severity 轉換：`max(0, (neg_ratio - 0.10) / 0.50)`，clamp 至 [0, 1]
...
|-----------|----------|------|
| < 10% | 0.0 | 正常波動範圍，不計入風險 |
| 10% ~ 60% | 0.0 → 1.0 | 線性爬升（每增加 1% 提升 0.02） |
| ≥ 60% | 1.0 | 最高風險 |
...
目前作為**觀察參考指標**，不計入 composite score。高訊息量可能代表議題密集或關係活躍；低訊息量加上其他高分指標，可能代表客戶已轉為冷漠。
...
### I6 語義熵時間序列（Semantic Entropy）

**代表意義**：以 Shannon 熵量化每個對話時間點的「語義多元程度」，追蹤對話主題的複雜度隨時間的變化趨勢。  
目前作為**探索性診斷指標**，不計入 composite score，但在 PDF 報告中以 sparkline 圖表呈現，協助識別對話結構性特徵。

#### 核心概念：字元 Bigram Shannon 熵

對每個 sliding window 內的訊息文字，計算**字元 bigram**（連續兩個字元）的 Shannon 熵：

```
H = -Σ p(bigram_i) × log₂(p(bigram_i))
```

其中 `p(bigram_i)` 為該 bigram 在視窗內所有文字中出現的機率。

- **熵高**（bits 值大）：視窗內文字使用了大量不同的 bigram，表示話題多元、用詞豐富，對話橫跨多個主題。
- **熵低**（bits 值小）：視窗內 bigram 高度重複，表示對話聚焦於單一主題或用詞單一（例如反覆討論同一問題）。

**選用字元 bigram 而非詞彙的理由**：中文斷詞需要外部依賴（jieba 等），而字元 bigram 在不依賴分詞器的情況下能有效捕捉詞彙多樣性，且對繁體中文效果良好。

#### 計算流程

**原始碼**：`src/metrics.py :: compute_i6_entropy_series()`
...
1. 過濾訊息：僅保留有文字內容的訊息（排除貼圖、圖片）
2. 對每一則訊息 `i`，取前 `window`（預設 5）則訊息為一個視窗
3. 計算視窗內所有文字的字元 bigram 熵，記錄為 `(timestamp, entropy_bits)`
4. 構成時序列表 `i6_entropy_series`

#### 衍生統計量

| 統計量 | 計算方式 | 說明 |
|--------|---------|------|
| `i6_mean_entropy` | `sum(entropies) / n` | 整段對話的平均熵值（bits） |
| `i6_entropy_slope` | `late_half_mean - early_half_mean` | 正值 = 後段熵高於前段（對話趨複雜）；負值 = 後段熵低（對話收斂） |
| `i6_peak_entropy_frac` | `peak_index / (n-1)` | 峰值出現位置，0.0 = 對話開頭，1.0 = 對話末尾 |
| `i6_escalation_fracs` | escalation 訊息在時序中的分數位置 | 用於圖表標記，比對熵峰與升級事件的時間關係 |

#### Slope 趨勢標籤

| slope 值 | 標籤 | 解讀 |
|----------|------|------|
| > +0.5 | ↑ 上升（對話趨複雜） | 對話後段出現新話題或情緒擴散，需關注 |
| +0.1 ~ +0.5 | ↗ 緩升 | 輕微多元化，可持續觀察 |
| -0.1 ~ +0.1 | → 穩定 | 話題一致，對話聚焦 |
| < -0.1 | ↘ 收斂（對話趨專一） | 後段討論集中至特定議題 |

#### 與其他指標的相關性解讀

I6 本身不直接代表風險，而是提供**結構性背景**：

- **I6 slope 高 + Tripwire 觸發**：升級前後對話出現語義擴散，客戶提出新議題或情緒轉向，是複合風險的訊號。
- **I6 峰值 ≈ 升級事件位置**（差距 < 20%）：熵峰與升級詞在時序上高度吻合，顯示升級事件帶來了對話主題的劇烈變化。
- **I6 slope 低但 I4 高**：客戶情緒持續負面但話題不變，可能是反覆投訴同一問題（固化型不滿）。
- **I6 slope 高但 I4 低**：對話內容多元但情緒尚可，可能是正常的多議題推進。

#### PDF 圖表說明

PDF 報告中的 sparkline 圖（`src/report_writer.py :: _entropy_chart()`）：

- **藍色折線**：entropy bits 隨訊息序號的變化
- **灰色虛線**：對話平均熵值（mean entropy 參考線）
- **紅色虛線**：升級詞觸發的位置（來自 `i6_escalation_fracs`）
- **X 軸**：從對話開始到結尾（序號歸一化）
- **Y 軸**：熵值（bits），標示最小值與最大值

圖表僅在對話有 ≥ 5 則有效文字訊息時顯示。

#### 輸出欄位

| 欄位 | 型別 | 說明 |
|------|------|------|
| `i6_entropy_series` | list[(datetime, float)] | 時序列表，每筆為 (時間戳, 熵值 bits) |
| `i6_mean_entropy` | float | 對話平均熵（bits） |
| `i6_entropy_slope` | float | 後半段均值 − 前半段均值 |
| `i6_peak_entropy_frac` | float | 峰值出現位置 [0.0, 1.0] |
| `i6_escalation_fracs` | list[float] | 升級訊息在序列中的分數位置 |
...
**注意**：I5（訊息量）與 I6（語義熵）為觀察性指標，不計入 composite score。
...
**場景 7：調整 I6 sliding window 大小**
```python
# src/metrics.py :: compute_metrics()
# 預設 window=5，縮小可增加對單一訊息的敏感度，放大可使曲線更平滑
i6_series, i6_mean, i6_slope, i6_peak_frac = compute_i6_entropy_series(messages, window=3)
```

**場景 8：調整 I4 負面情緒 warn 閾值**
```python
# src/metrics.py :: compute_metrics()
# 目前設定：warn=10%，crit=60%
m.i4_severity = min(1.0, max(0.0, (m.i4_neg_ratio - 0.10) / 0.50))
# 若想提高敏感度（5% 即觸發）：
m.i4_severity = min(1.0, max(0.0, (m.i4_neg_ratio - 0.05) / 0.55))
```
...
| 指標 | warn | crit | 單位 |
|------|------|------|------|
| I1 未回應年齡 | 30 分 | 360 分 | 業務分鐘 |
| I2 回應延遲 P90 | 120 分 | 480 分 | 業務分鐘 |
| I3 未解議題年齡 | 60 分 | 480 分 | 掛鐘分鐘 |
| I4 負面比率 | 10% | 60% | 比例 |
| Tripwire I1 硬上限 | — | 480 分 | 業務分鐘 |
| I6 slope（趨勢警示） | +0.1 | +0.5 | bits（參考用） |
